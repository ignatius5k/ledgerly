...
test("service-worker updates wait for an explicit activation request", async () => {
  ...
  assert.ok(worker.cacheAdditions.includes("./ledgerly-mark.png?v=57"));
  assert.ok(worker.cacheAdditions.includes("./eng-hoon-residences-logo.png?v=57"));
  assert.ok(worker.cacheAdditions.includes("./icon-192.png?v=57"));
  assert.ok(worker.cacheAdditions.includes("./icon-512.png?v=57"));
...
test("activation removes only previous Ledgerly shell caches", async () => {
  ...
  assert.equal(await (await worker.stores.get("invoice-studio-v57").get(runtimeUrl)).text(), "warmed PDF runtime");
...
test("only managed shell and runtime requests are cached and used offline", async () => {
  const shellUrl = `${SCOPE}app.js?v=57`;
  ...
  assert.deepEqual(worker.cachePuts, [{ cacheName: "invoice-studio-v57", key: shellUrl }]);
  ...
  assert.deepEqual(worker.cachePuts, [
    { cacheName: "invoice-studio-v57", key: shellUrl },
    { cacheName: "invoice-studio-v57", key: runtimeUrl },
  ]);
