...
test("service-worker updates wait for an explicit activation request", async () => {
  ...
  assert.ok(worker.cacheAdditions.includes("./ledgerly-mark.png?v=56"));
  assert.ok(worker.cacheAdditions.includes("./eng-hoon-residences-logo.png?v=56"));
  assert.ok(worker.cacheAdditions.includes("./icon-192.png?v=56"));
  assert.ok(worker.cacheAdditions.includes("./icon-512.png?v=56"));
...
test("activation removes only previous Ledgerly shell caches", async () => {
  ...
  assert.equal(await (await worker.stores.get("invoice-studio-v56").get(runtimeUrl)).text(), "warmed PDF runtime");
...
test("only managed shell and runtime requests are cached and used offline", async () => {
  const shellUrl = `${SCOPE}app.js?v=56`;
  ...
  assert.deepEqual(worker.cachePuts, [{ cacheName: "invoice-studio-v56", key: shellUrl }]);
  ...
  assert.deepEqual(worker.cachePuts, [
    { cacheName: "invoice-studio-v56", key: shellUrl },
    { cacheName: "invoice-studio-v56", key: runtimeUrl },
  ]);
