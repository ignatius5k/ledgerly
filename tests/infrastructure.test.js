...
test("activation removes only previous Invoice Studio caches", async () => {
  ...
  assert.equal(await (await worker.stores.get("invoice-studio-v41").get(runtimeUrl)).text(), "warmed PDF runtime");
...
test("only managed shell and runtime requests are cached and used offline", async () => {
  const shellUrl = `${SCOPE}app.js?v=41`;
  ...
  assert.deepEqual(worker.cachePuts, [{ cacheName: "invoice-studio-v41", key: shellUrl }]);
  ...
  assert.deepEqual(worker.cachePuts, [
    { cacheName: "invoice-studio-v41", key: shellUrl },
    { cacheName: "invoice-studio-v41", key: runtimeUrl },
  ]);
