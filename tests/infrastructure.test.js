...
test("activation removes only previous Invoice Studio caches", async () => {
  ...
  await activation;
  assert.deepEqual(worker.deletedCaches, ["invoice-studio-v1", "invoice-studio-v27", "invoice-studio-v28", "invoice-studio-v29", "invoice-studio-v30", "invoice-studio-v31", "invoice-studio-v32", "invoice-studio-v33", "invoice-studio-v34", "invoice-studio-v35", "invoice-studio-v36", "invoice-studio-v37", "invoice-studio-v38"]);
  assert.equal(await (await worker.stores.get("invoice-studio-v39").get(runtimeUrl)).text(), "warmed PDF runtime");
...
test("only managed shell and runtime requests are cached and used offline", async () => {
  const shellUrl = `${SCOPE}app.js?v=39`;
  ...
  assert.deepEqual(worker.cachePuts, [{ cacheName: "invoice-studio-v39", key: shellUrl }]);
  ...
  assert.deepEqual(worker.cachePuts, [
    { cacheName: "invoice-studio-v39", key: shellUrl },
    { cacheName: "invoice-studio-v39", key: runtimeUrl },
  ]);
